...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            .fc-toolbar.fc-header-toolbar { flex-direction: row; align-items: center; justify-content: space-between; gap: 0.2rem; margin-bottom: 10px !important; }
            .fc-toolbar-chunk { display: flex; align-items: center; gap: 2px; }
...
<body class="{{ (isset($isEss) && $isEss) ? 'bg-gray-50 font-sans w-full md:max-w-sm mx-auto min-h-screen flex flex-col shadow-lg border-x border-gray-100' : 'bg-gray-50 font-sans' }}">
    ...
    @else
        ...
        <div class="sticky top-0 bg-white/95 backdrop-blur-md z-20 border-b border-gray-200">
            <div class="p-3 flex items-center justify-between">
                ...
                <h1 class="font-bold text-base text-gray-800">Team Roster</h1>
                <div class="w-9"></div> 
            </div>
        </div>
    ...
    @endif
            ...
            <div class="{{ (isset($isEss) && $isEss) ? 'flex flex-col gap-3' : 'bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4' }}">
                ...
                <div class="w-full md:w-auto">
                    ...
                    <p class="text-sm text-gray-500">Manage shifts & schedules</p>
                </div>
                @endif
                ...
                <div class="{{ (isset($isEss) && $isEss) ? 'w-full' : 'w-full md:w-1/3' }}">
                        ...
                        <form action="{{ route('schedule') }}" method="GET" id="filterForm">
                            ...
                            @if(isset($outlets) && $outlets->count() > 0)
                                <div class="relative mt-2">
                                    <select name="outlet_id" onchange="this.form.submit()" class="w-full rounded-lg border-gray-300 shadow-sm p-2.5 pl-9 border focus:ring-2 focus:ring-indigo-500 font-medium text-gray-700 cursor-pointer bg-gray-50">
                    ...
                    @else
                        ...
                        <form action="{{ route('ess-coordinator-schedule') }}" method="GET">
                             <div class="relative">
                                <select name="outlet_id" onchange="this.form.submit()" class="w-full rounded-lg border-gray-300 shadow-sm p-2.5 pl-9 border focus:ring-2 focus:ring-indigo-500 font-medium text-gray-700 cursor-pointer bg-white text-sm">
                ...
                <div class="flex gap-2 w-full md:w-auto justify-end">
                    <a href="{{ route('shift') }}" class="px-4 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition font-bold flex items-center gap-2 text-sm whitespace-nowrap">
                        <i class="fas fa-clock"></i> Master Shifts
                    </a>
                    
                    @if($selectedBranchId)
                        <button id="addBtn" class="px-5 py-2.5 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-700 transition font-bold flex items-center gap-2 whitespace-nowrap">
                            <i class="fas fa-plus"></i> Assign
                        </button>
                    @else
                        <button disabled class="px-5 py-2.5 bg-gray-300 text-gray-500 rounded-lg cursor-not-allowed font-bold flex items-center gap-2 whitespace-nowrap" title="Select a branch first">
                            <i class="fas fa-plus"></i> Assign
                        </button>
                    @endif
                </div>
                ...
                <div class="{{ (isset($isEss) && $isEss) ? 'flex flex-col gap-4' : 'grid grid-cols-1 xl:grid-cols-3 gap-6' }}">
                    ...
                    <div class="{{ (isset($isEss) && $isEss) ? 'order-1' : 'xl:col-span-2 order-2' }} bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
                        <div class="p-3 border-b border-gray-100 bg-gray-50 flex flex-wrap justify-between items-center gap-2">
                            <h2 class="font-bold text-gray-700 text-sm">Calendar</h2>
                            <div class="flex flex-wrap gap-1 justify-end">
                                @foreach($shifts as $s)
                                    <div class="flex items-center gap-1 text-[10px] bg-white px-1.5 py-0.5 rounded border">
                                        <span class="w-2 h-2 rounded-full" style="background-color: {{ $s->color }}"></span> {{ $s->name }}
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="{{ (isset($isEss) && $isEss) ? 'order-2' : 'xl:col-span-1 order-1' }} bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 flex flex-col h-full">
                        <div class="p-4 border-b border-gray-100 bg-gray-50">
                            <h2 class="font-bold text-gray-700 text-sm">Assignments List</h2>
                        </div>
                        <div class="p-4 overflow-auto flex-grow">
                            <table id="myTable" class="w-full text-left">
                                <thead class="bg-gray-50 text-gray-600 text-[10px] uppercase tracking-wider">
                                    <tr>
                                        <th class="p-3 font-bold border-b border-gray-100">Employee</th>
                                ...
                                <tbody class="text-gray-700 text-xs divide-y divide-gray-100">
                                    @foreach ($schedules as $item)
                                        <tr class="hover:bg-indigo-50 transition">
                                            <td class="p-3">
                                                <div class="font-bold text-gray-900 text-xs">{{ $item->employee->name ?? 'Unknown' }}</div>
                                                @if(isset($isEss) && $isEss)
                                                @else
                                                    <div class="text-[9px] text-gray-400 mt-0.5">{{ $item->employee->branch->name ?? '-' }}</div>
                                                @endif
                                            </td>
                                            <td class="p-3 text-center">
                                                <div class="font-medium text-gray-800">{{ \Carbon\Carbon::parse($item->date)->format('d M') }}</div>
                                                <span class="inline-block mt-0.5 px-1.5 py-px rounded text-[9px] font-bold text-white" style="background-color: {{ $item->shift->color ?? '#ccc' }}">
                                                    {{ $item->shift->name ?? '-' }}
                                                </span>
                                            </td>
                                            <td class="p-3 text-center">
                                                <div class="flex justify-center items-center gap-1">
                                                    {{-- Edit --}}
                                                    <button class="editBtn w-7 h-7 flex items-center justify-center bg-indigo-50 text-indigo-600 rounded hover:bg-indigo-100 transition"
                                                        data-id="{{ $item->id }}"
                                                    ...
                                                    {{-- Delete --}}
                                                    @php
                                                        $deleteRoute = (isset($isEss) && $isEss) 
                                                            ? route('ess-coordinator-schedule-destroy', $item->id)
                                                            : route('delschedule', $item->id); 
                                                    @endphp
                                                    <form method="post" action="{{ $deleteRoute }}" class="inline deleteForm">
                                                        @csrf @method('DELETE')
                                                        <button type="button" class="delete-confirm w-7 h-7 flex items-center justify-center bg-red-50 text-red-600 rounded hover:bg-red-100 transition">
            ...
            @else
                <!-- EMPTY STATE -->
                <div class="flex flex-col items-center justify-center h-80 bg-white rounded-xl shadow-sm border border-dashed border-gray-300 text-center p-6">
                    <div class="bg-indigo-50 p-3 rounded-full mb-3">
                        <i class="fas fa-building text-3xl text-indigo-400"></i>
                    </div>
                    <h3 class="text-lg font-bold text-gray-800">
                        {{ (isset($isEss) && $isEss) ? 'No Schedule Data' : 'Select a Branch' }}
                    </h3>
                    <p class="text-gray-500 mt-1 text-xs max-w-xs">
                        {{ (isset($isEss) && $isEss) 
                           ? 'No schedule data found for your branch/outlet.' 
                           : 'Please select a branch to view schedules.' }}
                    </p>
                </div>
            @endif
    ...
    @else
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg md:max-w-sm mx-auto p-4 z-30">
            <button id="addBtnESS" class="w-full py-3.5 bg-indigo-600 text-white font-bold rounded-xl shadow-lg hover:bg-indigo-700 transition flex items-center justify-center gap-2 transform active:scale-95">
                <i class="fas fa-plus-circle"></i> Assign Schedule
            </button>
        </div>
        @endif
    @endif
    ...
    <div id="addModal" class="hidden fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
        <div class="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-lg shadow-2xl relative transform transition-all scale-100 h-[85vh] sm:h-auto flex flex-col">
            ...
            <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
                ...
                <button id="closeAddModal" class="w-8 h-8 flex items-center justify-center rounded-full bg-white text-gray-400 hover:text-gray-600 shadow-sm border border-gray-100 transition"><i class="fas fa-times"></i></button>
            </div>
                ...
                <form id="addForm" method="post" 
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <label class="block text-sm font-semibold text-gray-700">Select Employees</label>
                            ...
                            <div class="flex items-center gap-2">
                                <label class="inline-flex items-center cursor-pointer">
                                    ...
                                    <span class="ml-1 text-xs text-gray-600 font-bold">All</span>
                                </label>
                            </div>
                        </div>

                        <div class="relative mb-2">
                            <input type="text" id="employeeSearch" placeholder="Search name..." class="w-full rounded-lg border-gray-300 p-2 pl-8 text-xs focus:ring-2 focus:ring-indigo-500">
                        ...
                        <div class="border border-gray-200 rounded-xl overflow-hidden max-h-48 overflow-y-auto custom-scrollbar bg-gray-50 p-1">
                            @foreach ($employees as $emp)
                                <label class="employee-item flex items-center p-2.5 hover:bg-white rounded-lg cursor-pointer transition border border-transparent hover:border-gray-200 group">
                                    <input type="checkbox" name="employee_ids[]" value="{{ $emp->id }}" class="emp-checkbox w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 mr-3">
                                    <div>
                                        <div class="text-sm font-bold text-gray-700 group-hover:text-indigo-700">{{ $emp->name }}</div>
                                        <div class="text-[10px] text-gray-500 flex items-center gap-1">
                                            @if(isset($isEss) && $isEss)
                                                {{ $emp->position->name ?? '-' }}
                                            @else
                                                <i class="fas fa-store text-[9px]"></i> {{ $emp->outlet->name ?? $emp->position->name ?? '-' }}
                                            @endif
                                        </div>
                                    </div>
                                </label>
                            ...
                            @if($employees->isEmpty())
                                <p class="text-center text-xs text-gray-400 py-4">No employees found.</p>
                            @endif
                        </div>
                        <p class="text-[10px] text-gray-400 mt-1 text-right">
                            Selected: <span id="selectedCount" class="font-bold text-indigo-600">0</span>
                        </p>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">From Date</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">To Date</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">Shift Template</label>
                        <div class="grid grid-cols-2 gap-3 max-h-40 overflow-y-auto p-1 custom-scrollbar">
                            @foreach($shifts as $shift)
                                <label class="flex items-center p-3 border rounded-xl cursor-pointer hover:bg-gray-50 transition border-gray-200 has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50">
            ...
            <div class="p-4 border-t border-gray-100 bg-white rounded-b-2xl">
                <button type="submit" form="addForm" class="w-full py-3.5 bg-indigo-600 text-white font-bold rounded-xl shadow-lg hover:bg-indigo-700 transition flex items-center justify-center gap-2 transform active:scale-95">
    ...
    <div id="editModal" class="hidden fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
        <div class="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-lg shadow-2xl relative transform transition-all scale-100">
            <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
                 <h2 class="text-lg font-bold text-gray-800 flex items-center gap-2"><i class="fas fa-edit text-blue-600"></i> Edit Schedule</h2>
                 <button id="closeModal" class="w-8 h-8 flex items-center justify-center rounded-full bg-white text-gray-400 hover:text-gray-600 shadow-sm border border-gray-100 transition"><i class="fas fa-times"></i></button>
            </div>
            <div class="p-6">
                <form id="editForm" method="post" class="space-y-5">
                    @csrf @method('put')
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">Employee</label>
                        <select id="editEmployeeId" name="employee_id" class="w-full rounded-xl border-gray-300 shadow-sm p-3 border focus:ring-2 focus:ring-blue-500 text-sm">
                            @foreach ($employees as $emp)
                    ...
                    <div>
                        ...
                        <input type="date" id="editDate" name="date" class="w-full rounded-xl border-gray-300 shadow-sm p-3 border bg-gray-50 text-gray-500 text-sm" readonly>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">Shift</label>
                        <select id="editShiftId" name="shift_id" class="w-full rounded-xl border-gray-300 shadow-sm p-3 border focus:ring-2 focus:ring-blue-500 text-sm" required>
                            @foreach ($shifts as $shift)
                                <option value="{{ $shift->id }}">{{ $shift->name }} ({{ substr($shift->start_time, 0, 5) }} - {{ substr($shift->end_time, 0, 5) }})</option>
                            @endforeach
                        </select>
                    </div>
                    <button type="submit" class="w-full py-3.5 bg-blue-600 text-white font-bold rounded-xl shadow-md hover:bg-blue-700 transition flex justify-center items-center gap-2 transform active:scale-95">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.editBtn', function() {
                ...
                let actionUrl = `${baseUrl}/${btn.data('id')}`;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function(info) {
                    ...
                    $('#editForm').attr('action', `${calBaseUrl}/${info.event.id}`);
                    $('#editModal').removeClass('hidden');
                }
            });
