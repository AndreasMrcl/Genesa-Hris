...
<body
            ...
            <div
                ...
                <div class="flex gap-3 pl-2">
                    <div class="flex flex-col items-center min-w-[3.5rem]">
                        ...
                        <span
                            class="text-[9px] font-bold text-gray-400 mt-1 uppercase">{{ $startDate->format('D') }}</span>
                    </div>
                    ...
                    <div class="flex-grow">
                        <div class="flex justify-between items-start mb-1">
                            ...
                            <span
                                class="px-2 py-0.5 rounded text-[9px] font-bold uppercase border {{ $statusColor }} flex items-center gap-1">
                                <i class="fas {{ $statusIcon }}"></i> {{ $item->status }}
                            </span>
                        </div>

                        <div
                            class="bg-gray-50 rounded-lg p-2 border border-gray-100 mt-2 flex justify-between items-center">
                            <div class="text-xs text-gray-600 font-medium">
                                {{ $startDate->format('d M') }} - {{ $endDate->format('d M Y') }}
                            </div>
                            <span
                                class="bg-white px-2 py-0.5 rounded text-[10px] font-bold text-gray-700 border border-gray-200 shadow-sm">
                                {{ $duration }} Days
                            </span>
                        </div>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg md:max-w-sm mx-auto p-4 z-30">
        <button id="addBtn"
            class="w-full py-3 bg-yellow-500 text-white font-bold rounded-xl shadow-md hover:bg-yellow-600 transition flex items-center justify-center gap-2 transform active:scale-95">
        ...
        <div
            ...
            <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
                ...
                <button id="closeAddModal"
                    class="w-8 h-8 flex items-center justify-center rounded-full bg-white text-gray-400 hover:text-gray-600 shadow-sm transition">
            ...
            <div class="p-6 overflow-y-auto flex-grow">
                <form id="addForm" method="post" action="{{ route('req-leave') }}" class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="start_date"
                                class="w-full rounded-xl border-gray-300 shadow-sm p-3 border focus:ring-2 focus:ring-yellow-500 transition text-sm"
                                required>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <select name="type"
                            class="w-full rounded-xl border-gray-300 shadow-sm p-3 border focus:ring-2 focus:ring-yellow-500 bg-white text-sm"
                            required>
                    ...
                    <div>
                        ...
                        <textarea name="note" rows="4"
                            class="w-full rounded-xl border-gray-300 shadow-sm p-3 border focus:ring-2 focus:ring-yellow-500 text-sm"
                            placeholder="Please describe your reason / note..." required></textarea>
                    </div>
                </form>
            </div>
            ...
            <div class="p-4 border-t border-gray-100 bg-white rounded-b-2xl">
                <button type="submit" form="addForm"
                    class="w-full py-3.5 bg-yellow-500 text-white font-bold rounded-xl shadow-lg hover:bg-yellow-600 transition flex items-center justify-center gap-2 transform active:scale-95">
