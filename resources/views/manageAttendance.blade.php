            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                ...
                <form action="{{ route('manageattendance') }}" method="GET" class="flex flex-wrap items-end gap-4">
                    <div>
                        ...
                        <input type="date" name="start" value="{{ $start ?? '' }}" class="rounded-lg border-gray-300 shadow-sm p-2.5 border focus:ring-2 focus:ring-blue-500" required>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 mb-1">End Date</label>
                        <input type="date" name="end" value="{{ $end ?? '' }}" class="rounded-lg border-gray-300 shadow-sm p-2.5 border focus:ring-2 focus:ring-blue-500" required>
                    </div>
                    <button type="submit" class="px-6 py-2.5 bg-gray-800 text-white rounded-lg hover:bg-gray-900 transition font-bold shadow-md">
                        Load Data
                    </button>
                    
                    @if($start && $end)
                        <a href="{{ route('attendance') }}" class="px-6 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition font-bold">
                            Cancel
                        </a>
                    @endif
                </form>
                    ...
                    <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
                        <div class="p-4 bg-blue-50 border-b border-blue-100 flex justify-between items-center sticky top-0 z-10">
                            <div>
                                ...
                                <p class="text-xs text-blue-600">Data will be saved for period: <strong>{{ $start }}</strong> to <strong>{{ $end }}</strong></p>
                            </div>
                            <button type="submit" class="px-8 py-2.5 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 transition font-bold flex items-center gap-2">
                                    ...
                                    @foreach ($employees as $emp)
                                        ...
                                        <tr class="hover:bg-blue-50 transition group">
                                            ...
                                            <td class="p-2 text-center bg-orange-50/50">
                                                <input type="number" min="0" name="data[{{ $emp->id }}][late]" 
                                            ...
                                            <td class="p-2 text-center bg-red-50/30">
                                                <input type="number" min="0" name="data[{{ $emp->id }}][alpha]" 
