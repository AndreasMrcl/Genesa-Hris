            ...
            <div class="flex justify-between items-center bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 class="font-bold text-2xl text-gray-800 flex items-center gap-2">
                        <i class="fas fa-history text-indigo-600"></i> Audit Trail
                    </h1>
                            ...
                            @forelse ($logs as $log)
                                <tr class="hover:bg-gray-50 transition duration-150 align-top">
                                    ...
                                    <td class="p-4">
                                        ...
                                        <div class="flex items-center gap-3 p-2 bg-gray-50 rounded-lg border border-gray-100 hover:bg-white hover:shadow-sm transition">
                                            <div class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 text-xs font-bold shrink-0 uppercase shadow-sm">
                                                {{ substr($log->user->name ?? 'S', 0, 1) }}
                                            </div>
                                            <div class="overflow-hidden">
                                                <p class="font-bold text-xs text-gray-700 truncate" title="{{ $log->user->name ?? 'System' }}">
                                                    {{ $log->user->name ?? 'System' }}
                                                </p>
                                                <p class="text-[10px] text-gray-400 truncate">ID: #{{ $log->user_id ?? '-' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="3" class="p-12 text-center">
                                        <div class="flex flex-col items-center justify-center opacity-50">
                                            ...
                                            <p class="text-sm text-gray-500 mt-1">System activities will be recorded here automatically.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
